var formFiliacaoIntivoCpf = $('form[name="formFiliacaoInativoCpf"]');
var errorDados = $('.alert-danger', formFiliacaoIntivoCpf);
formFiliacaoIntivoCpf.validate({
    errorElement: 'span',
    ...
    submitHandler: function () {
        errorDados.hide();
        swal({
            title: "Filiação de Membro Inativo",
            text: "Você tem certeza que deseja iniciar este processo de filiação?",
            type: "warning",
            ...
            if (isConfirm) {
                formFiliacaoIntivoCpf.ajaxSubmit({
                    url: 'modulo/loja/grande_secretaria/processos/filiacaoinativo/control_filiacaoinativo.php',
                    data: {acao: 'sendFiliacaoInativoCpf'},
                    type: 'POST',
                    beforeSend: function () {
                    ...
                    success: function (res) {
                        ...
                        if(resposta.quiteplacet === true){
                            var url = 'modulo/loja/grande_secretaria/processos/filiacaoinativo/filiacaoinativo_membro.php?id='+resposta.id;
                        }else{
                            var url = 'modulo/loja/grande_secretaria/processos/filiacaoinativo/filiacaoinativo_quiteplacet_membro.php?id='+resposta.id;
                        }
...
var FormWizardFiliacaoInativo = function () {
    return {
        init: function () {
            if (!jQuery().bootstrapWizard) {
                return;
            }
            var form = $('#formFiliaçãoInativo');
            var id_membro = form.attr('name');
            var error = $('.alert-danger', form);
            ...
            var handleTitle = function (tab, navigation, index) {
                var total = navigation.find('li').length;
                var current = index + 1;
                $('.step-title', $('#form_wizard_filiacaoinativo')).text('Passo ' + (index + 1) + ' de ' + total);
                jQuery('li', $('#form_wizard_filiacaoinativo')).removeClass("done");
                var li_list = navigation.find('li');
                for (var i = 0; i < index; i++) {
                    jQuery(li_list[i]).addClass("done");
                }
                if (current === 1) {
                    $('#form_wizard_filiacaoinativo').find('.button-previous').hide();
                } else {
                    $('#form_wizard_filiacaoinativo').find('.button-previous').show();
                }
                if (current >= total) {
                    $('#form_wizard_filiacaoinativo').find('.button-next').hide();
                    $('#form_wizard_filiacaoinativo').find('.button-submit').show();
                    displayConfirm();
                } else {
                    $('#form_wizard_filiacaoinativo').find('.button-next').show();
                    $('#form_wizard_filiacaoinativo').find('.button-submit').hide();
                }
                App.scrollTo($('.page-title'));
            }
            $('#form_wizard_filiacaoinativo').bootstrapWizard({
                'nextSelector': '.button-next',
                'previousSelector': '.button-previous',
                onTabClick: function (tab, navigation, index, clickedIndex) {
                    return false;
                ...
                onTabShow: function (tab, navigation, index) {
                    ...
                    var current = index + 1;
                    var $percent = (current / total) * 100;
                    $('#form_wizard_filiacaoinativo').find('.progress-bar').css({
                        width: $percent + '%'
                    });
                }
            });
            ...
            $('#form_wizard_filiacaoinativo').find('.button-previous').hide();
            $('#form_wizard_filiacaoinativo .button-submit').click(function () {
                swal({
                    title: "Solicitar Filiação",
                    text: "Você tem certeza que deseja solicitar este processo de filiação?",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonClass: "btn-warning",
                    confirmButtonText: "Sim, solicitar",
                    ...
                    if (isConfirm) {
                        form.ajaxSubmit({
                            url: 'modulo/loja/grande_secretaria/processos/filiacaoinativo/control_filiacaoinativo.php',
                            data: {acao: 'sendFiliacaoInativo', id: id_membro},
                            type: 'POST',
                            beforeSend: function () {
...
jQuery(document).ready(function () {
    FormWizardFiliacaoInativo.init();
});
