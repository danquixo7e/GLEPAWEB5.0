        ...
        if($readMembro){
            ...
            if(!$readCpf){
                ...
                if(!$readEmail){
                    ...
                    $readProcesso = read('glepaweb_processos', "WHERE id_membro = '$id' AND id_tipo = '2' AND cancelado != '1'");
                    if (!$readProcesso) {
                        $p['data_cerimonial'] = filter_input(INPUT_POST, 'data_cerimonial');
                        $p['data_cerimonial'] = formDate($p['data_cerimonial']);
                        $interticio = date('Y-m-d', strtotime("-5 months", strtotime($p['data_cerimonial'])));
                        $readHistorico = read('glepaweb_historico',"WHERE id_membro = '$id' AND id_tipo = '1'");
                        if($readHistorico){
                            foreach ($readHistorico as $historico);
                            if ($historico['data'] <= $interticio) {
                                ...
                                $retorno['mensagem'] = 'A data informada é incompatível com o intertício de 6 meses';
                            }
                        } else {
                            $retorno['sucesso'] = false;
                            $retorno['mensagem'] = 'Não foi possível encontrar a data de iniciação deste membro';
                        }
                    }else{
                        $retorno['sucesso'] = false;
                        $retorno['mensagem'] = 'Já existe uma solicitação de elevação para este membro';
                    }
                }else{
                    $retorno['sucesso'] = false;
            ...
            $retorno['sucesso'] = false;
            $retorno['mensagem'] = 'Não foi possível solicitar a elevação deste membro';
        }
