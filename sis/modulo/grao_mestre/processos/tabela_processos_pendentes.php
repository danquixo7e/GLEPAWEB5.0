...
$readProcessos = read('glepaweb_processos',"WHERE id_status = '5'");
if ($readProcessos) {
    $records = array();
    $records["data"] = array();
    foreach ($readProcessos as $processos => $value) {
        $records["data"][] = array(
            ...
            '<a href="modulo/grao_mestre/processos/processos_editar.php?id='.$value['id'].'" class="btn green m-icon m-icon-only ajaxify"><i class="m-icon-swapright m-icon-white"></i></a>'
        );
    }
