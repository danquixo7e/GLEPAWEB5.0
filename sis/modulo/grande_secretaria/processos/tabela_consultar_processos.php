...
if ($readProcessos) {
    ...
    foreach ($readProcessos as $processos => $value) {
        $records["data"][] = array(
            ...
            '<a href="modulo/grande_secretaria/processos/processos_consultar.php?id='.$value['id'].'" class="btn yellow m-icon m-icon-only ajaxify"><i class="m-icon-swapright m-icon-white"></i></a>'
        );
    }
